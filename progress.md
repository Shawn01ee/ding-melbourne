...
## Continuation: stable iOS stop transitions and AutoFill isolation
- Kept the smallest keyboard-open Visual Viewport height for the lifetime of each focus session. Safari can now add or remove its QuickType/Contact AutoFill row between stops without moving the stats or stop console up and down.
- Moved the invisible keyboard input out of the address-like stop panel into a document-level portal, replaced its dynamic street-name label with a generic typing-game label, and disabled correction, capitalization, spelling, and autocomplete hints. The visible stop panel keeps its focus styling and remains the tap target.
- Verification: at the iPhone 16 Pro CSS width of 402px, completed one stop, continued typing the next, then simulated the AutoFill row disappearing by growing the visual viewport by 44px. The journey board, stats, and stop console retained identical coordinates, input focus and tram motion continued, horizontal overflow stayed at zero, and no console/page errors occurred. All 78 tests and the production PWA build pass.

## Continuation: distinct short targets for same-name stops
- Confirmed Route 1's `Arts Precinct/Sturt St #17` and `Arts Precinct/St Kilda Rd #14` are separate official platforms about 308m apart, rather than a duplicated GTFS record.
- Standard mode now detects when one direction contains the same short stop name at different intersections. Only those collisions expand to a slash-free intersection target, so Route 1 presents `Arts Precinct Sturt St` followed by `Arts Precinct St Kilda Rd`; ordinary short targets stay unchanged and Driver mode still uses the full official intersection tier.
- Verification: started Route 1 at Stop 17, typed both Arts Precinct targets continuously, and reached Federation Square with character combo preserved and zero errors. Desktop and iPhone 16 Pro-width screenshots were inspected; both long targets fit without clipping or horizontal overflow. All 80 tests and the production PWA build pass.

## Continuation: forward-only network previews
- New request: stop Routes 12, 57, 59, and 96 from moving forward and then backward.
- Root cause: the live game map already sanitised GTFS retraces, but Network Map → Route Focus animated its tram over the uncleaned first-direction shape. These four first-direction shapes all contain vertices removed by the existing forward-only cleanup.
- Added shared-frame path projection that decides cleanup at route-local scale, then places the retained vertices back into the whole-network geographic frame. Network overview lines and focused moving trams now use the cleaned geometry.
- Added named regression coverage for Routes 12, 57, 59, and 96.
- Verification: 82/82 tests, production build, and `git diff --check` pass. The bundled game client rendered the 24-line network with zero console/page errors. Chromium then opened all four focused routes and confirmed each animated tram uses exactly the same cleaned geometry as its displayed line (12: 249 points, 57: 212, 59: 225, 96: 267); all four screenshots were inspected.
- Follow-up verification focused only on the user-supplied Network Map view, not gameplay: at 2048×1152 in night mode, sampled the moving tram at 2%, 20%, 40%, 60%, 80%, and 98% of the full 14-second Route Focus animation for Routes 12, 57, 59, and 96. Every sample stayed exactly on the cleaned displayed rail (0.00px error), all four captures were inspected, the 24-line All Lines view rendered correctly, and the browser reported no console/page errors.
- User reported the issue still occurred and supplied Route 96 near the end of its Network Route Focus run. The earlier 2%–98% audit missed the 100% cycle boundary: `repeatCount="indefinite"` sent the preview tram back to its origin after arrival. Changed Route Focus to explicitly begin a fresh one-way 14-second run for each selected route, repeat once, and freeze at the destination.
- Post-arrival Chromium verification now covers Routes 12, 57, 59, and 96: each starts at its origin, reaches its destination with 0.00px error just after 14 seconds, and remains there with 0.00px error at 28 seconds. Route switching starts a fresh run. The Route 96 Blyth St post-arrival screenshot and bundled-client All Lines screenshot were inspected; no console/page errors occurred. All 82 tests, the production build, and `git diff --check` pass.
- A follow-up Route 96 screenshot exposed a separate focus-layer bug: the selected magenta Route 96 crossed a dimmed Route 109, whose transparent hit stroke still intercepted hover and displayed the native `Route 109 · Port Melbourne - Box Hill` tooltip. Route Focus now removes titles and pointer interaction from every unselected line while keeping All Lines fully interactive.
- Chromium verified that Route 96 Focus exposes only the selected Route 96 title and that both Route 109 hit/color strokes have `pointer-events: none`; switching to All Lines restores all 24 titles and Route 109's stroke interaction. The focused Route 96 and bundled All Lines screenshots were inspected, no console/page errors occurred, and all 82 tests plus the production build pass.
- Production diagnosis: `https://ding-melbourne.vercel.app/` still serves the pre-fix `index-BGZy1ZV5.js` bundle, which contains `repeatCount:"indefinite"`; the current verified local build is therefore not visible in the installed/live app. A production deploy was not authorized by the external-action reviewer and requires explicit user approval before proceeding.
- The prior production bundle was subsequently deployed, but a new Route 96 close-up showed that native SVG `animateMotion` still selected a visibly wrong tangent at the tight CBD bend. Runtime sampling found heading/movement disagreement on all four reported routes (up to 175°), even after positional retraces were removed.
